...
if __name__ == '__main__':
    ...
    # 加载数据
    try:
        with open('qpcr_test/test_20251223/config.json', 'r') as f:
    ...
    try:
        df_raw = pd.read_csv('qpcr_test/test_20251223/k6_cldn_mbd_time_3_calculate_2.txt', sep='\t', skiprows=1)
    ...
    # 运行分析
    try:
        results = main(df_raw, json_data, control_sample='116_Ctrl')
        print("\n✓ 分析成功完成！")
    except Exception as e:
        print(f"\n✗ 分析过程中出错: {e}")
        import traceback
